...
public class App {
	...
	private void viewPendingRequests() {
        ...
        if (!pendingTransfers.isEmpty()) {
            ...
            if (transferToUpdate == null) {
                System.out.println("You cannot update that transfer.\nNo transfer has been updated.");
            } else {
    ...
    private Account selectAnAccountForTransfer() {
        ...
        HashMap<Long, Account> userAccounts;

        long accountSelection = -1;
        while (accountSelection != 0) {
            userAccounts = consoleService.printAllUsersWithHashMap(accountList, userAccount.getUser().getId());

            accountSelection = consoleService.promptForMenuSelection("Please select a user: ");

            if (userAccounts.get(accountSelection) != null) {
                return userAccounts.get(accountSelection);
            } else {
                System.out.println("Invalid selection!");
            }
        }
